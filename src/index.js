...
// Load environment file (supports QA)
const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const envFile = process.env.ENV_FILE;
const isQa = (process.env.NODE_ENV || '').toLowerCase() === 'qa';
const envCandidates = [
    envFile ? path.resolve(__dirname, '..', envFile) : null,
    isQa ? path.resolve(__dirname, '../.env.qa') : null,
    path.resolve(__dirname, '../.env'),
].filter(Boolean);
const envPath = envCandidates.find(p => fs.existsSync(p));
dotenv.config({ path: envPath });

// Import DB after env is loaded (DATABASE_URL is read on import)
