...
psgc.get('/barangays', authMiddleware, async (c) => {
  try {
    ...
    const countResult = await pool.query(
      `SELECT COUNT(*) as count FROM psgc ${whereClause}`,
      params
    );
...
psgc.post('/user/:userId/assignments', authMiddleware, async (c) => {
  try {
    ...
    const psgcCheck = await pool.query(
      'SELECT id FROM psgc WHERE id = ANY($1)',
      [psgcIds]
    );
    ...
    for (const psgcId of psgcIds) {
      ...
      const existing = await pool.query(
        'SELECT id, deleted_at FROM user_psgc_assignments WHERE user_id = $1 AND psgc_id = $2',
        [userId, psgcId]
      );
      ...
      if (existing.rows.length > 0) {
        ...
        if (existing.rows[0].deleted_at) {
          await pool.query(
            'UPDATE user_psgc_assignments SET deleted_at = NULL, assigned_at = NOW(), assigned_by = $1 WHERE id = $2',
        ...
        await pool.query(
          'INSERT INTO user_psgc_assignments (id, user_id, psgc_id, assigned_at, assigned_by) VALUES (gen_random_uuid(), $1, $2, NOW(), $3)',
          [userId, psgcId, currentUser.sub]
        );
        assigned++;
      }
    }
...
psgc.delete('/user/:userId/assignments/:psgcId', authMiddleware, async (c) => {
  try {
    ...
    const existing = await pool.query(
      'SELECT id FROM user_psgc_assignments WHERE user_id = $1 AND psgc_id = $2 AND deleted_at IS NULL',
      [userId, psgcId]
    );
    ...
    // Soft delete
    await pool.query(
      'UPDATE user_psgc_assignments SET deleted_at = NOW() WHERE id = $1',
      [existing.rows[0].id]
    );
