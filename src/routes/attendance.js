...
attendance.post('/check-in', authMiddleware, auditMiddleware('attendance'), async (c) => {
    try {
        ...
        const existing = await pool.query('SELECT * FROM attendance WHERE user_id = $1 AND date = $2', [user.sub, today]);
        if (existing.rows.length > 0) {
            return c.json({ message: 'Already checked in for today', attendance: mapRowToAttendance(existing.rows[0]) }, 400);
        }
        // Create new attendance record
        const result = await pool.query(`INSERT INTO attendance (id, user_id, date, time_in, location_in_lat, location_in_lng, notes)
       VALUES (gen_random_uuid(), $1, $2, NOW(), $3, $4, $5)
       RETURNING *`, [user.sub, today, validated.latitude, validated.longitude, validated.notes]);
...
attendance.post('/check-out', authMiddleware, auditMiddleware('attendance'), async (c) => {
    try {
       ...
       WHERE id = $4
       RETURNING *`, [validated.latitude, validated.longitude, validated.notes, attendanceRecord.id]);
...
attendance.get('/today', authMiddleware, async (c) => {
    try {
        const user = c.get('user');
        const today = getLocalDateString();
        const result = await pool.query('SELECT * FROM attendance WHERE user_id = $1 AND date = $2', [user.sub, today]);
...
attendance.get('/history', authMiddleware, async (c) => {
    try {
        ...
        // Get total count
        const countResult = await pool.query('SELECT COUNT(*) as count FROM attendance WHERE user_id = $1', [targetUserId]);
        const totalItems = parseInt(countResult.rows[0].count);
        // Get paginated results
        const result = await pool.query(`SELECT a.*, u.first_name, u.last_name, u.email
       FROM attendance a
...
attendance.get('/', authMiddleware, async (c) => {
    try {
        ...
        // Get total count
        const countResult = await pool.query(`SELECT COUNT(*) as count FROM attendance a ${whereClause}`, params);
        const totalItems = parseInt(countResult.rows[0].count);
        // Get paginated results
        const result = await pool.query(`SELECT a.*, u.first_name, u.last_name, u.email
       FROM attendance a
