...
targets.get('/', authMiddleware, async (c) => {
    try {
        ...
        const conditions = ['period = $1', 'year = $2'];
        ...
        const result = await pool.query(`SELECT t.*, u.first_name, u.last_name FROM targets t
...
targets.get('/current', authMiddleware, async (c) => {
    try {
        ...
        const result = await pool.query(`SELECT t.*, u.first_name, u.last_name FROM targets t
       JOIN users u ON u.id = t.user_id
       WHERE t.user_id = $1 AND t.period = 'monthly' AND t.year = $2 AND t.month = $3`, [userId, year, month]);
        // Get actual progress
        ...
        (SELECT COUNT(*) FROM touchpoints WHERE user_id = $1 AND type = 'Visit' AND EXTRACT(MONTH FROM date) = $2 AND EXTRACT(YEAR FROM date) = $3) as actual_visits`, [userId, month, year]);
...
targets.post('/', authMiddleware, auditMiddleware('target'), async (c) => {
    try {
        ...
        // Upsert target
        const result = await pool.query(`INSERT INTO targets (id, user_id, period, year, month, week, target_clients, target_touchpoints, target_visits)
       ...
       RETURNING *`, [validated.user_id, validated.period, validated.year, validated.month, validated.week,
...
targets.delete('/:id', authMiddleware, auditMiddleware('target'), async (c) => {
    try {
        ...
        const existing = await pool.query('SELECT * FROM targets WHERE id = $1', [id]);
