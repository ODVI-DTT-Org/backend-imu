...
agencies.get('/', authMiddleware, async (c) => {
    try {
        ...
        // Get total count
        const countResult = await pool.query(`SELECT COUNT(*) as count FROM agencies ${whereClause}`, params);
        const totalItems = parseInt(countResult.rows[0].count);
        // Get paginated results
        const result = await pool.query(`SELECT * FROM agencies ${whereClause}
       ORDER BY name ASC
...
agencies.post('/', authMiddleware, requireAnyRole('admin', 'staff'), auditMiddleware('agency'), async (c) => {
    try {
        ...
        const result = await pool.query(`INSERT INTO agencies (id, name, code, address)
       VALUES (gen_random_uuid(), $1, $2, $3)
       RETURNING *`, [validated.name, validated.code, validated.address]);
...
agencies.put('/:id', authMiddleware, requireAnyRole('admin', 'staff'), auditMiddleware('agency'), async (c) => {
    try {
        ...
        const result = await pool.query(`UPDATE agencies SET ${updateFields.join(', ')} WHERE id = $${paramIndex} RETURNING *`, updateValues);
        return c.json(mapRowToAgency(result.rows[0]));
    }
    catch (error) {
        if (error instanceof z.ZodError) {
