...
dashboard.get('/', authMiddleware, async (c) => {
    try {
        ...
        const clientStats = await pool.query(`SELECT
       ...
       FROM clients
       WHERE 1=1 ${user.role === 'field_agent' ? 'AND user_id = $1' : ''}`, caravanParams);
        // Get touchpoint statistics for the period
        const touchpointStats = await pool.query(`SELECT
        COUNT(*) as total_touchpoints,
        COUNT(*) FILTER (WHERE type = 'Visit') as visits,
        COUNT(*) FILTER (WHERE type = 'Call') as calls
       FROM touchpoints
       WHERE date >= $1 AND date <= $2 ${caravanFilter}`, [monthStart, monthEnd, ...caravanParams]);
        // Get itinerary statistics
        const itineraryStats = await pool.query(`SELECT
       ...
       FROM itineraries
       WHERE scheduled_date >= $1 AND scheduled_date <= $2 ${caravanFilter}`, [monthStart, monthEnd, ...caravanParams]);
        // Get caravan statistics (admin only)
        let caravanStats = { total_caravans: 0, active_caravans: 0 };
        if (user.role === 'admin' || user.role === 'staff') {
            const caravanResult = await pool.query(`SELECT
       ...
       ORDER BY date DESC
       LIMIT 10`, user.role === 'field_agent' ? [user.sub, user.sub] : []);
        // Get clients by agency breakdown
        const clientsByAgency = await pool.query(`SELECT a.name as agency_name, COUNT(c.id) as client_count
       FROM clients c
       LEFT JOIN agencies a ON a.id = c.agency_id
       ${user.role === 'field_agent' ? 'WHERE c.user_id = $1' : ''}
       GROUP BY a.name
       ORDER BY client_count DESC
       LIMIT 5`, caravanParams);
        return c.json({
            period: {
...
dashboard.get('/performance', authMiddleware, async (c) => {
    try {
        ...
        const dailyTouchpoints = await pool.query(`SELECT date, COUNT(*) as count
       FROM touchpoints
       WHERE user_id = $1
         AND date >= CURRENT_DATE - INTERVAL '30 days'
       GROUP BY date
       ORDER BY date DESC`, [caravanId]);
        // Get touchpoint type breakdown
        const touchpointTypes = await pool.query(`SELECT type, COUNT(*) as count
       FROM touchpoints
       WHERE user_id = $1
         AND date >= CURRENT_DATE - INTERVAL '30 days'
       GROUP BY type`, [caravanId]);
        // Get client conversion rate
        const conversionStats = await pool.query(`SELECT
        COUNT(*) FILTER (WHERE client_type = 'POTENTIAL') as potential,
        COUNT(*) FILTER (WHERE client_type = 'EXISTING') as existing
       FROM clients
       WHERE user_id = $1`, [caravanId]);
        return c.json({
