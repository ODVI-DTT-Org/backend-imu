...
touchpointsAnalytics.get('/', authMiddleware, async (c) => {
    try {
       ...
       ORDER BY t.touchpoint_number`, params);
        ...
        const trendsResult = await pool.query(`SELECT
        t.date,
        COUNT(*) as count,
        COUNT(*) FILTER (WHERE t.status = 'Completed') as completed
       FROM touchpoints t
       ...
       ORDER BY t.date ASC`, params);
        ...
        const caravanPerformanceResult = await pool.query(`SELECT
       ...
       ORDER BY completed DESC, total DESC`, params);
        ...
        const statusDistributionResult = await pool.query(`SELECT
        t.status,
        COUNT(*) as count
       FROM touchpoints t
       LEFT JOIN clients c ON c.id = t.client_id
       ...
       ORDER BY count DESC`, params);
        const totalStatus = statusDistributionResult.rows.reduce((sum, row) => sum + (parseInt(row.count) || 0), 0);
