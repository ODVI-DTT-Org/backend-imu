...
touchpointReasons.post('/', authMiddleware, async (c) => {
    try {
        ...
        const result = await pool.query(`INSERT INTO touchpoint_reasons (code, label, touchpoint_type, role, category, sort_order, color)
       VALUES ($1, $2, $3, $4, $5, $6, $7)
       RETURNING id, code, label, touchpoint_type, role, category, sort_order, color`, [code, label, touchpoint_type, role, category || null, sort_order, color]);
...
touchpointReasons.put('/:id', authMiddleware, async (c) => {
    try {
        ...
        const result = await pool.query(`UPDATE touchpoint_reasons
       SET ${updates.join(', ')}
       WHERE id = $${paramIndex}
       RETURNING id, code, label, touchpoint_type, role, category, sort_order, color, is_active`, params);
...
touchpointReasons.delete('/:id', authMiddleware, async (c) => {
    try {
        ...
        const id = c.req.param('id');
        const result = await pool.query(`DELETE FROM touchpoint_reasons WHERE id = $1 RETURNING id`, [id]);
        if (result.rows.length === 0) {
            return c.json({ message: 'Touchpoint reason not found' }, 404);
        }
