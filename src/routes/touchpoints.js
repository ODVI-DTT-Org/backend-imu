 ...
 */
function getExpectedTouchpointType(touchpointNumber) {
 ...
 */
async function getNextTouchpointNumber(clientId) {
    const result = await pool.query(`SELECT COUNT(DISTINCT touchpoint_number) as count
     FROM touchpoints
 ...
 */
function validateTouchpointSequence(touchpointNumber, touchpointType) {
 ...
 */
function canCreateTouchpoint(userRole, touchpointNumber, touchpointType) {
...
function mapRowToTouchpoint(row) {
    return {
        id: row.id,
...
touchpoints.get('/', authMiddleware, async (c) => {
    try {
        ...
        // Get total count
        const countResult = await pool.query(`SELECT COUNT(*) as count FROM touchpoints t ${whereClause}`, params);
...
touchpoints.get('/reasons', authMiddleware, async (c) => {
    try {
        const result = await pool.query(`SELECT id, code, label, color, sort_order
...
touchpoints.get('/next/:clientId', authMiddleware, async (c) => {
    try {
        ...
        const existingResult = await pool.query(`SELECT touchpoint_number, type, date, edit_status
       FROM touchpoints
       WHERE client_id = $1
       AND edit_status IN ('approved', 'pending_approval')
       ORDER BY touchpoint_number ASC`, [clientId]);
...
touchpoints.post('/', authMiddleware, auditMiddleware('touchpoint'), async (c) => {
    try {
        ...
        const sequenceValidation = validateTouchpointSequence(validated.touchpoint_number, validated.type);
        ...
        if (validated.type === 'Call') {
            ...
            // Get existing touchpoints for this client
            const existingTouchpointsResult = await pool.query(`SELECT touchpoint_number FROM touchpoints WHERE client_id = $1`, [validated.client_id]);
        ...
        const existingTouchpoint = await pool.query(`SELECT id, touchpoint_number, type
       FROM touchpoints
       WHERE client_id = $1
       AND touchpoint_number = $2
       LIMIT 1`, [validated.client_id, validated.touchpoint_number]);
        ...
        const result = await pool.query(`INSERT INTO touchpoints (
...
touchpoints.put('/:id', authMiddleware, auditMiddleware('touchpoint'), async (c) => {
    try {
        ...
        const updateResult = await pool.query(`UPDATE touchpoints SET ${updates.join(', ')} WHERE id = $${valueIndex} RETURNING *`, [...values, id]);
        return c.json({
