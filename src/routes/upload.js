...
upload.get('/pending', async (c) => {
    ...
    try {
        ...
        for (const table of tables) {
            ...
            const result = await pool.query(`SELECT COUNT(*) as count FROM ${table} WHERE ${columnName} = $1`, [user.sub]);
...
upload.post('/file', async (c) => {
    ...
    try {
        ...
        const entityId = body['entity_id']; // Optional: link to entity (client, touchpoint, etc.)
        const entityType = body['entity_type']; // Optional: entity type (client, touchpoint, etc.)
        ...
        const fileRecord = await pool.query(`INSERT INTO files (filename, original_filename, mime_type, size, url, storage_key, uploaded_by, entity_type, entity_id)
