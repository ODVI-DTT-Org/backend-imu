...
const VALID_ROLES = ['admin', 'area_manager', 'assistant_area_manager', 'caravan', 'tele', 'staff', 'field_agent'];
const MANAGER_ROLES = ['admin', 'area_manager', 'assistant_area_manager'];
const CARAVAN_ROLES = ['caravan', 'field_agent'];
const TELE_ROLES = ['tele'];
// Validation schemas
...
users.get('/', authMiddleware, requireRole('admin', 'staff'), async (c) => {
    try {
        ...
        const result = await pool.query(`SELECT id, email, first_name, last_name, role, phone, avatar_url, created_at, updated_at
...
users.post('/', authMiddleware, auditMiddleware('user'), requireRole('admin'), async (c) => {
    try {
        ...
        if (CARAVAN_ROLES.includes(validated.role)) {
        ...
        const result = await pool.query(`INSERT INTO users (id, email, password_hash, first_name, last_name, role, phone, avatar_url)
       VALUES (gen_random_uuid(), $1, $2, $3, $4, $5, $6, $7)
       RETURNING id, email, first_name, last_name, role, phone, avatar_url, created_at, updated_at`, [validated.email, password_hash, validated.first_name, validated.last_name,
            validated.role, validated.phone, validated.avatar_url]);
        const newUser = result.rows[0];
        // Create user profile with manager assignments
        await pool.query(`INSERT INTO user_profiles (id, user_id, name, email, role, area_manager_id, assistant_area_manager_id)
       VALUES (gen_random_uuid(), $1, $2, $3, $4, $5, $6)`, [newUser.id, `${validated.first_name} ${validated.last_name}`, validated.email,
...
users.put('/:id', authMiddleware, auditMiddleware('user'), async (c) => {
    try {
        ...
        if (validated.role === 'caravan' || CARAVAN_ROLES.includes(validated.role)) {
            if (!validated.area_manager_id && !validated.assistant_area_manager_id) {
        ...
        for (const [key, dbField] of Object.entries(fieldMappings)) {
            if (key in validated) {
                ...
                updateValues.push(validated[key]);
                paramIndex++;
            }
        }
        ...
        const result = await pool.query(`UPDATE users SET ${updateFields.join(', ')} WHERE id = $${paramIndex}
       RETURNING id, email, first_name, last_name, role, phone, avatar_url, created_at, updated_at`, updateValues);
...
users.post('/:id/change-password', authMiddleware, async (c) => {
    try {
        ...
        await auditAuth.passwordChange(currentUser.sub, c.req.header('x-forwarded-for') || c.req.header('x-real-ip'));
...
users.delete('/:id', authMiddleware, auditMiddleware('user'), requireRole('admin'), async (c) => {
    try {
        ...
        const existing = await pool.query('SELECT id, email, first_name, last_name, role FROM users WHERE id = $1', [id]);
        if (existing.rows.length === 0) {
            return c.json({ message: 'User not found' }, 404);
        }
        const oldUser = existing.rows[0];
...
users.post('/:id/municipalities/bulk', authMiddleware, requireAnyRole(...MANAGER_ROLES), async (c) => {
    try {
        ...
        const result = await pool.query(`UPDATE user_locations
       SET deleted_at = NOW()
       WHERE user_id = $1
         AND TRIM(municipality_id) = ANY($2)
         AND deleted_at IS NULL
       RETURNING id`, [userId, validated.municipality_ids.map(m => m.trim())]);
        return c.json({
...
users.delete('/:id/municipalities/:municipalityId', authMiddleware, requireAnyRole(...MANAGER_ROLES), async (c) => {
    try {
        ...
        const existing = await pool.query('SELECT id, deleted_at FROM user_locations WHERE user_id = $1 AND TRIM(municipality_id) = TRIM($2)', [userId, municipalityId]);
        if (existing.rows.length === 0) {
