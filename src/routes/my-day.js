...
myDay.post('/add-client', authMiddleware, async (c) => {
    try {
        ...
        const clientCheck = await pool.query('SELECT * FROM clients WHERE id = $1', [validated.client_id]);
        ...
        const existing = await pool.query('SELECT * FROM itineraries WHERE client_id = $1 AND user_id = $2 AND scheduled_date = $3', [validated.client_id, user.sub, today]);
        if (existing.rows.length > 0) {
            return c.json({ message: 'Client already in today\'s itinerary' }, 400);
        }
        // Add to itinerary
        const result = await pool.query(`INSERT INTO itineraries (id, client_id, user_id, scheduled_date, scheduled_time, priority, notes, status, created_by)
...
myDay.delete('/remove-client/:id', authMiddleware, async (c) => {
    try {
        ...
        const today = getLocalDateString();
        const result = await pool.query(`DELETE FROM itineraries
       WHERE client_id = $1 AND user_id = $2 AND scheduled_date = $3
       RETURNING *`, [clientId, user.sub, today]);
...
myDay.get('/status/:clientId', authMiddleware, async (c) => {
    try {
        ...
        const today = getLocalDateString();
        const result = await pool.query('SELECT * FROM itineraries WHERE client_id = $1 AND user_id = $2 AND scheduled_date = $3', [clientId, user.sub, today]);
        return c.json({
...
myDay.get('/tasks', authMiddleware, async (c) => {
    try {
        ...
        const itinerariesResult = await pool.query(`SELECT i.*, c.first_name, c.last_name, c.email, c.phone, c.client_type,
              a.name as agency_name
       FROM itineraries i
       JOIN clients c ON c.id = i.client_id
       LEFT JOIN agencies a ON a.id = c.agency_id
       WHERE i.user_id = $1 AND i.scheduled_date = $2
       ORDER BY i.scheduled_time ASC NULLS LAST, i.priority DESC`, [caravanId, today]);
        // Get today's completed touchpoints
        const touchpointsResult = await pool.query(`SELECT t.id, t.client_id, t.touchpoint_number, t.type, t.reason, t.time_arrival, t.time_departure,
              c.first_name, c.last_name, c.client_type
       FROM touchpoints t
       JOIN clients c ON c.id = t.client_id
       WHERE t.user_id = $1 AND t.date = $2
       ORDER BY t.created_at DESC`, [user.sub, today]);
...
myDay.post('/tasks/:id/start', authMiddleware, async (c) => {
    try {
        ...
        const existing = await pool.query('SELECT * FROM itineraries WHERE id = $1 AND user_id = $2', [taskId, user.sub]);
        if (existing.rows.length === 0) {
        ...
        const result = await pool.query(`UPDATE itineraries SET status = 'in_progress', updated_at = NOW()
...
myDay.post('/tasks/:id/complete', authMiddleware, async (c) => {
    try {
        ...
        const result = await pool.query(`UPDATE itineraries SET status = 'completed', updated_at = NOW()
       WHERE id = $1 RETURNING *`, [taskId]);
        return c.json({
...
myDay.post('/clients/:id/time-in', authMiddleware, async (c) => {
    try {
        ...
        const clientCheck = await pool.query('SELECT * FROM clients WHERE id = $1 AND user_id = $2', [clientId, user.sub]);
        if (clientCheck.rows.length === 0) {
            return c.json({ message: 'Client not found or not assigned to you' }, 404);
        }
        const now = new Date();
        ...
        const existing = await pool.query('SELECT * FROM touchpoints WHERE client_id = $1 AND user_id = $2 AND date = $3', [clientId, user.sub, today]);
        let result;
        if (existing.rows.length > 0) {
            // Update existing touchpoint with time-in
            result = await pool.query(`UPDATE touchpoints SET time_arrival = $1, latitude = $2, longitude = $3, updated_at = NOW()
         WHERE id = $4 RETURNING *`, [timeIn, validated.latitude, validated.longitude, existing.rows[0].id]);
        }
        else {
            // Create new touchpoint with time-in
            const tpResult = await pool.query('SELECT COUNT(*) + 1 as next FROM touchpoints WHERE client_id = $1', [clientId]);
            const nextNumber = Math.min(parseInt(tpResult.rows[0].next) || 1, 7);
            result = await pool.query(`INSERT INTO touchpoints (id, client_id, user_id, touchpoint_number, type, date, time_arrival, latitude, longitude)
         VALUES (gen_random_uuid(), $1, $2, $3, 'Visit', $4, $5, $6, $7) RETURNING *`, [clientId, user.sub, nextNumber, today, timeIn, validated.latitude, validated.longitude]);
        }
        return c.json({
...
myDay.post('/visits', authMiddleware, async (c) => {
    try {
        ...
        const today = getLocalDateString();
        // Check for existing touchpoint today
        const existing = await pool.query('SELECT * FROM touchpoints WHERE client_id = $1 AND user_id = $2 AND date = $3', [validated.client_id, user.sub, today]);
        let result;
        if (existing.rows.length > 0) {
            // Update existing touchpoint
            result = await pool.query(`UPDATE touchpoints SET
        ...
        WHERE id = $15 RETURNING *`, [
            ...
            result = await pool.query(`INSERT INTO touchpoints (
        ...
        await pool.query(`UPDATE itineraries SET status = 'completed', updated_at = NOW()
       WHERE client_id = $1 AND user_id = $2 AND scheduled_date = $3 AND status != 'completed'`, [validated.client_id, user.sub, today]);
        return c.json({
...
myDay.get('/stats', authMiddleware, async (c) => {
    try {
        ...
        const touchpointsResult = await pool.query(`SELECT COUNT(*) as total,
              COUNT(*) FILTER (WHERE type = 'Visit') as visits,
              COUNT(*) FILTER (WHERE type = 'Call') as calls
       FROM touchpoints WHERE user_id = $1 AND date >= $2`, [user.sub, startDate]);
        const clientsResult = await pool.query(`SELECT COUNT(DISTINCT client_id) as unique_clients
       FROM touchpoints WHERE user_id = $1 AND date >= $2`, [user.sub, startDate]);
        const itinerariesResult = await pool.query(`SELECT COUNT(*) as total,
              COUNT(*) FILTER (WHERE status = 'completed') as completed
       FROM itineraries WHERE user_id = $1 AND scheduled_date >= $2`, [user.sub, startDate]);
