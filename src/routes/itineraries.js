...
function mapRowToItinerary(row) {
    return {
        id: row.id,
        user_id: row.user_id,
...
itineraries.get('/', authMiddleware, async (c) => {
    try {
        ...
        // Get total count
        const countResult = await pool.query(`SELECT COUNT(*) as count FROM itineraries i ${whereClause}`, params);
...
itineraries.post('/', authMiddleware, auditMiddleware('itinerary'), async (c) => {
    try {
        ...
        const clientCheck = await pool.query(`SELECT id, municipality FROM clients WHERE id = $1`, [validated.client_id]);
        if (clientCheck.rows.length === 0) {
            return c.json({ message: 'Client not found' }, 404);
        }
        // Check if itinerary already exists for this client, user, and date
        const existingCheck = await pool.query(`SELECT id FROM itineraries
       WHERE client_id = $1 AND user_id = $2 AND scheduled_date = $3`, [validated.client_id, user.sub, validated.scheduled_date]);
        ...
        const result = await pool.query(`INSERT INTO itineraries (
...
itineraries.put('/:id', authMiddleware, auditMiddleware('itinerary'), async (c) => {
    try {
        ...
        const existing = await pool.query(`SELECT i.*, c.municipality as client_municipality
       FROM itineraries i
       LEFT JOIN clients c ON c.id = i.client_id
       WHERE i.id = $1`, [id]);
        ...
        const result = await pool.query(`UPDATE itineraries SET ${updateFields.join(', ')} WHERE id = $${paramIndex} RETURNING *`, updateValues);
