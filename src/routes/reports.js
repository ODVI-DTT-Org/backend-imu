...
reports.get('/agent-performance', authMiddleware, async (c) => {
    try {
       ...
       ORDER BY total_touchpoints DESC`, params);
        return c.json({
            period,
...
reports.get('/touchpoint-summary', authMiddleware, async (c) => {
    try {
        ...
        // By type
        const byType = await pool.query(`SELECT type, COUNT(*) as count FROM touchpoints ${whereClause} GROUP BY type`, params);
        // By status
        const byStatus = await pool.query(`SELECT status, COUNT(*) as count FROM touchpoints ${whereClause} GROUP BY status`, params);
        // By reason (top 10)
        const byReason = await pool.query(`SELECT reason, COUNT(*) as count FROM touchpoints ${whereClause} GROUP BY reason ORDER BY count DESC LIMIT 10`, params);
        // By touchpoint number
        const byNumber = await pool.query(`SELECT touchpoint_number, COUNT(*) as count FROM touchpoints ${whereClause} GROUP BY touchpoint_number ORDER BY touchpoint_number`, params);
        return c.json({
            period,
            start_date: startDate.toISOString(),
            end_date: endDate.toISOString(),
...
reports.get('/attendance-summary', authMiddleware, async (c) => {
    try {
       ...
       ORDER BY present_days DESC`, params);
        return c.json({
            period,
            start_date: startDate.toISOString(),
            end_date: endDate.toISOString(),
...
reports.get('/target-achievement', authMiddleware, async (c) => {
    try {
        ...
        // Get target
        const targetResult = await pool.query(`SELECT * FROM targets WHERE user_id = $1 AND period = 'monthly' AND year = $2 AND month = $3`, [userId, year, month]);
        // Get actuals
        const actualsResult = await pool.query(`SELECT
        ...
        (SELECT COUNT(*) FROM touchpoints WHERE user_id = $1 AND type = 'Visit'
         AND EXTRACT(MONTH FROM date) = $2 AND EXTRACT(YEAR FROM date) = $3) as actual_visits`, [userId, month, year]);
...
reports.get('/conversion', authMiddleware, async (c) => {
    try {
        ...
        // Conversion funnel
        const funnelResult = await pool.query(`SELECT
       ...
       ${whereClause}`, params);
        ...
        const conversionByTouchpoint = await pool.query(`SELECT
        t.touchpoint_number,
       ...
       ORDER BY t.touchpoint_number`, params);
        // Conversion by reason (which touchpoint reasons lead to conversions)
        const conversionByReason = await pool.query(`SELECT
        t.reason,
        COUNT(DISTINCT CASE WHEN c.client_type = 'EXISTING' THEN c.id END) as conversions
       FROM clients c
       JOIN touchpoints t ON t.client_id = c.id
...
reports.get('/area-coverage', authMiddleware, async (c) => {
    try {
        ...
        // Coverage by city
        const byCity = await pool.query(`SELECT
        a.city,
       ...
       ${whereClause}
       GROUP BY a.city
       ORDER BY touchpoints DESC`, params);
        // Coverage by province
        const byProvince = await pool.query(`SELECT
        a.province,
        COUNT(DISTINCT t.id) as touchpoints,
        COUNT(DISTINCT t.client_id) as unique_clients
       FROM touchpoints t
       ...
       ORDER BY touchpoints DESC`, params);
        ...
        const coverageStats = await pool.query(`SELECT
...
reports.get('/export', authMiddleware, async (c) => {
    try {
        ...
        switch (reportType) {
            case 'touchpoints':
                const tpResult = await pool.query(`SELECT t.id, t.date, t.type, t.reason, t.status, t.notes,
           ...
           ORDER BY t.date DESC`, [startDate, endDate]);
                csvData = 'ID,Date,Type,Reason,Status,Client,Agent,Notes\n';
                csvData += tpResult.rows.map(r => `"${r.id}","${r.date}","${r.type}","${r.reason}","${r.status}","${r.client_first_name} ${r.client_last_name}","${r.agent_first_name} ${r.agent_last_name}","${(r.notes || '').replace(/"/g, '""')}"`).join('\n');
                filename = `touchpoints_${period}.csv`;
                break;
            case 'clients':
                const clientResult = await pool.query(`SELECT c.id, c.first_name, c.last_name, c.email, c.phone, c.client_type,
                  c.product_type, c.market_type, c.created_at,
                  u.first_name as agent_first_name, u.last_name as agent_last_name
           FROM clients c
           LEFT JOIN users u ON u.id = c.user_id
           WHERE c.created_at >= $1 AND c.created_at <= $2
           ORDER BY c.created_at DESC`, [startDate, endDate]);
                csvData = 'ID,First Name,Last Name,Email,Phone,Type,Product Type,Market Type,Agent,Created\n';
                csvData += clientResult.rows.map(r => `"${r.id}","${r.first_name}","${r.last_name}","${r.email || ''}","${r.phone || ''}","${r.client_type}","${r.product_type || ''}","${r.market_type || ''}","${r.agent_first_name || ''} ${r.agent_last_name || ''}","${r.created_at}"`).join('\n');
                filename = `clients_${period}.csv`;
                break;
            case 'attendance':
                const attResult = await pool.query(`SELECT a.id, a.date, a.check_in, a.check_out, a.status,
                  u.first_name, u.last_name
           FROM attendance a
           JOIN users u ON u.id = a.user_id
           WHERE a.date >= $1 AND a.date <= $2
           ORDER BY a.date DESC`, [startDate, endDate]);
                csvData = 'ID,Date,Agent,Check In,Check Out,Status\n';
                csvData += attResult.rows.map(r => `"${r.id}","${r.date}","${r.first_name} ${r.last_name}","${r.check_in || ''}","${r.check_out || ''}","${r.status}"`).join('\n');
