...
class StorageService {
    ...
    async uploadToS3(file, key, mimetype) {
        try {
            const buffer = Buffer.isBuffer(file) ? file : Buffer.from(await file.arrayBuffer());
            // AWS S3 API call
    ...
    async uploadToR2(file, key, mimetype) {
        try {
            const buffer = Buffer.isBuffer(file) ? file : Buffer.from(await file.arrayBuffer());
    ...
    async uploadToSupabase(file, key, mimetype) {
        try {
            const buffer = Buffer.isBuffer(file) ? file : Buffer.from(await file.arrayBuffer());
            const response = await fetch(`${process.env.SUPABASE_URL}/storage/v1/object/${this.bucket}/${key}`, {
                method: 'POST',
                headers: {
