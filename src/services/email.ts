...
const templates = {
  ...
  approvalRequest: (approverName: string, requesterName: string, approvalType: string, clientName: string, approvalUrl: string) => ({
  ...
  approvalResult: (userName: string, approvalType: string, clientName: string, status: string, reason?: string) => ({
...
class EmailService {
  ...
  private async sendViaResend(to: string | string[], subject: string, html: string, text?: string, from?: string): Promise<EmailResult> {
    try {
  ...
  private async sendViaSendGrid(to: string | string[], subject: string, html: string, text?: string, from?: string): Promise<EmailResult> {
    try {
  ...
  private consoleSend(to: string | string[], subject: string, html: string): EmailResult {
    console.log('\n========== EMAIL ==========');
  ...
  private mockSend(to: string | string[], subject: string, html: string): EmailResult {
    // For testing - returns success without doing anything
    return { success: true, messageId: `mock-${crypto.randomUUID()}` };
  }

  // Template helpers
  async sendPasswordReset(email: string, resetUrl: string, userName: string): Promise<EmailResult> {
  ...
  async sendWelcome(email: string, userName: string, loginUrl: string): Promise<EmailResult> {
