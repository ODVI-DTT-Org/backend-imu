...
export async function cleanupOldAuditLogs(retentionDays: number = 90): Promise<{ deleted: number; error?: string }> {
  try {
...
export function startScheduledCleanup(intervalMs: number = 24 * 60 * 60 * 1000) { // Default: daily
  if (cleanupInterval) {
...
export type AuditSource = 'mobile' | 'web' | 'api' | 'unknown';

// Detect source from user-agent string
export function detectSource(userAgent?: string): AuditSource {
...
export async function auditLog(entry: AuditLogEntry): Promise<void> {
  try {
    await pool.query(
      `INSERT INTO audit_logs (user_id, action, entity, entity_id, old_values, new_values, ip_address, user_agent, metadata)
...
export function auditMiddleware(entity: AuditEntity, customAction?: AuditAction) {
...
export const auditAuth = {
  login: (userId: string, ipAddress?: string,
 userAgent?: string, success: boolean = true) =>
    auditLog({ userId, action: 'login', entity: 'auth', ipAddress, userAgent, metadata: { success, source: detectSource(userAgent) } }),
  logout: (userId: string, ipAddress?: string, userAgent?: string) =>
    auditLog({ userId, action: 'logout', entity: 'auth', ipAddress, userAgent, metadata: { source: detectSource(userAgent) } }),
  passwordReset: (userId: string, ipAddress?: string, userAgent?: string) =>
    auditLog({ userId, action: 'password_reset', entity: 'auth', ipAddress, userAgent, metadata: { source: detectSource(userAgent) } }),
  passwordChange: (userId: string, ipAddress?: string, userAgent?: string) =>
    auditLog({ userId, action: 'password_change', entity: 'auth', ipAddress, userAgent, metadata: { source: detectSource(userAgent) } }),
};

// Audit approval helpers
export const auditApproval = {
  approve: (userId: string, approvalId: string, clientId: string, notes?: string, userAgent?: string) =>
    auditLog({ userId, action: 'approve', entity: 'approval', entityId: approvalId, userAgent, metadata: { clientId, notes, source: detectSource(userAgent) } }),
  reject: (userId: string, approvalId: string, clientId: string, reason: string, userAgent?: string) =>
    auditLog({ userId, action: 'reject', entity: 'approval', entityId: approvalId, userAgent, metadata: { clientId, reason, source: detectSource(userAgent) } }),
};
