/**
 * Update clients with PSGC location data
 * This script updates existing clients with region, province, and municipality values
 */
...
async function updateClientsWithPSGC() {
  ...
  try {
    ...
    console.log(`Updating ${clientLocations.length} client locations...`);
    ...
    for (let i = 0; i < clientLocations.length; i++) {
      ...
      // Find clients that match this location (by existing municipality text)
      const clientsResult = await client.query(
        `SELECT id, first_name, last_name, municipality, region, province
         FROM clients
      ...
      if (clientsResult.rows.length === 0) {
        console.log(`⚠️  No clients found for: ${location.municipality}`);
        continue;
      }
        ...
        await client.query(
          `UPDATE clients
           SET region = $1,
               ...
               updated_at = NOW()
           WHERE id = $4`,
          [location.region, location.province, psgc.id, clientRow.id]
        );
        updateCount++;
        console.log(`✅ Updated: ${clientRow.first_name} ${clientRow.last_name} → ${location.municipality}, ${location.province}`);
      }
    }
    ...
    console.log(`⚠️  PSGC not found: ${notFoundCount} locations`);
    console.log(`Total locations processed: ${clientLocations.length}`);
  } catch (error: any) {
